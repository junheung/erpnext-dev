...
graph TB
    subgraph Docker["Docker 컨테이너"]
        Frontend["erpnext-frontend<br/>포트: 8300:8300"]
        Frappe["erpnext-frappe<br/>포트: 8100:8000, 9000:9000"]
        RedisCache["erpnext-redis-cache<br/>redis:7-alpine"]
        RedisQueue["erpnext-redis-queue<br/>redis:7-alpine"]
        MariaDB["erpnext-mariadb<br/>포트: 3306:3306"]
    end
    
    subgraph Local["로컬 개발 환경"]
        NextJS["Next.js 개발 서버<br/>포트: 3000"]
    end
    
    NextJS -->|API 요청| Frappe
    Frontend -->|API 요청| Frappe
    Frappe -->|데이터 저장/조회| MariaDB
    Frappe -->|캐시| RedisCache
    Frappe -->|작업 큐| RedisQueue
    
    style Frontend fill:#e1f5ff
    style Frappe fill:#fff4e1
...
│              Docker Containers (5)                     │
├────────────────────────────────────────────────────────┤
│  • erpnext-frontend      (port: 8300:8300)             │
│  • erpnext-frappe        (port: 8100:8000, 9000:9000)  │
│  • erpnext-redis-cache   (redis:7-alpine)              │
│  • erpnext-redis-queue   (redis:7-alpine)              │
│  • erpnext-mariadb       (port: 3306:3306)             │
└────────────────────────────────────────────────────────┘
                          ▲
                          │ API 요청
                          │
...
연결 관계:
  Next.js (3000) ──API 요청──> Frappe (8100)
  Frontend (8300) ──API 요청──> Frappe (8100)
  Frappe ──데이터 저장/조회──> MariaDB (3306)
  Frappe ──캐시──> Redis Cache
  Frappe ──작업 큐──> Redis Queue
```

**주요 서비스 용도:**

- **erpnext-frontend** (포트 8300:8300):
  - Next.js 기반 프론트엔드 애플리케이션
  - 사용자 인터페이스 제공 및 Frappe 백엔드 API와 통신
  - 개발 모드에서 HMR(Hot Module Replacement) 지원
  - 포트 형식: `호스트포트:컨테이너포트` (8300:8300 = 호스트 8300 → 컨테이너 8300)

- **erpnext-frappe** (포트 8100:8000, 9000:9000):
  - ERPNext/Frappe 백엔드 서버 (Python 기반)
  - REST API 제공, 비즈니스 로직 처리, 데이터베이스 관리
  - 포트 형식: `호스트포트:컨테이너포트`
    - `8100:8000`: 호스트의 8100 포트 → 컨테이너 내부의 8000 포트 (HTTP 웹 인터페이스 및 API)
    - `9000:9000`: 호스트의 9000 포트 → 컨테이너 내부의 9000 포트 (SocketIO 실시간 통신)
  - **왜 포트가 다른가?** 컨테이너 내부에서는 8000 포트로 실행되지만, 호스트에서는 8100 포트로 접근합니다. 이를 통해 여러 서비스를 동시에 실행하거나 포트 충돌을 방지할 수 있습니다.

**Redis 서비스 용도:**
- **redis-cache**: 데이터 캐싱, 세션 관리, API 응답 캐시, 함수 결과 캐싱으로 데이터베이스 부하 감소 및 응답 속도 향상
- **redis-queue**: 백그라운드 작업 큐, 비동기 작업 처리(이메일 발송, 리포트 생성, 데이터 동기화 등), 작업 스케줄링

> **참고**: Mermaid 다이어그램이 보이지 않는 경우:
> - VS Code: "Markdown Preview Mermaid Support" 확장 설치
> - GitHub: 자동으로 렌더링됨
> - [Mermaid Live Editor](https://mermaid.live/)에서 확인 가능
...
REM 4. ERPNext 백엔드 서버 시작
REM http://localhost:8100 로 접속이 안 되면 아래의 서버 한 번 더 실행해 주세요 
docker-compose exec -d --user frappe frappe bash -c "cd /workspace/frappe-bench && bench start"

REM 5. 새로운 터미날을 열어서 새로 개발한 프론트엔드 실행
REM http://localhost:3000 로 접속이 가능합니다. 
cd apps/erpnext-frontend
npm install
npm run dev
```

**또는 수동 설정:(리눅스 혹은 맥북 기준) **
...
**테스트 실행 전 확인사항:**
1. ERPNext 백엔드 서버가 실행 중이어야 합니다: `http://localhost:8100`
2. 프론트엔드 개발 서버가 실행 중이어야 합니다: `http://localhost:3000`
3. Docker 컨테이너들이 모두 실행 중이어야 합니다: `docker-compose ps`

**테스트 타임아웃 오류 발생 시:**
- 백엔드 서버 확인: `docker-compose logs frappe`
- 프론트엔드 서버 확인: 브라우저에서 `http://localhost:3000` 접속 테스트
- 서버 재시작: `docker-compose restart frappe`
