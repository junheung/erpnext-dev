...
export function SalesOrderItemTable({
  ...
  const handleItemSelect = async (index: number, itemCode: string) => {
    ...
    if (!selectedItem) {
      console.warn('[SalesOrderItemTable] 선택된 코드에 해당하는 품목을 찾지 못했습니다.', itemCode);
      return;
    }
    ...
    try {
      ...
      console.warn('[SalesOrderItemTable] 단건 조회 실패, 리스트 데이터로 계속 진행합니다.', e);
    }
    const newItems = [...items];
    ...
    onItemsChange(newItems);
    console.log('[SalesOrderItemTable] 품목 선택 완료', { index, itemCode, applied: newItems[index] });
  };
        ...
        <Table>
          <TableHeader>
            <TableRow className="bg-gray-200">
              ...
              <TableHead className="w-[140px]">납품예정일 <span className="text-red-500">*</span></TableHead>
